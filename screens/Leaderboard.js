...
const Leaderboard = ({ navigation }) => {
    ...
    const [previousOffset, setPreviousOffset] = useState({ x: 0, y: 0 });

    const panResponder = useRef(
        PanResponder.create({
            onStartShouldSetPanResponder: () => true,
            onPanResponderMove: (_, gesture) => {
                pan.x = previousOffset.x + gesture.dx;
                pan.y = previousOffset.y + gesture.dy;
                setPreviousOffset({ ...pan });
            },
            onPanResponderRelease: (_, gesture) => {
                setPreviousOffset({
                    x: previousOffset.x + gesture.dx,
                    y: previousOffset.y + gesture.dy,
                });
            },
        })
    ).current;
